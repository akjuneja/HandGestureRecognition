...
import numpy as np
from src.learning_models.snail_network.network_modules import temporal_data_encoder, AttentionBlock, TCBlock
from src.learning_models.neural_network_model.TemporalBlock import TemporalBlock
...
class SnailFewShot(nn.Module):
    def __init__(self, N, K, feature_dim, use_cuda=True):
        ...
        self.encoder = TemporalBlock(63, feature_dim=feature_dim)
        num_channels = feature_dim + N
    ...
    def forward(self, input, labels):
        x = self.encoder(input) # episode_len*batch_size x feature_dim x 1
